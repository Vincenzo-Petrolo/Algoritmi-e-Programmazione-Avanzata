...
void aggiungi_valori(char *decisione,collana_t *collana);
void collana_init(collana_t * collana);
void collana_destroy(collana_t* collana);
void collana_display(int* pietre,int n,int val);
void disp_ripet(wrapper_t *db);
boolean check(int *pietre,int n,collana_t *collana);
int calcola_valore(collana_t *collana);
boolean verifica_ripetizioni(wrapper_t* db,pietra_t parametro);
/**Prototypes*/
...
int main(int argc, char *argv[]) {
	...
	char menu[][30] = {"smeraldi", "zaffiri","rubini","topazi","val_s","val_z","val_r","val_t","max_rip","calcola"};
	...
	db.collana->pietre = (int *) malloc(somma_pietre*sizeof(int)); //creo una collana di pietre
...
void disp_ripet(wrapper_t *db) {
	...
	for (i = 0; i < __N_TIPI_PIETRE__; i++) {
		if ((db->sol[db->pos-1] == ZAFFIRO || db->sol[db->pos-1] == TOPAZIO) && (i == ZAFFIRO || i == RUBINO)){
