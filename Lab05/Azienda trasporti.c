        ...
        switch (richiesta_campo_ordinamento) {
            case r_date: {         /**0 Ordinamento per date**/
                if (vettori->caso_heap_insuff != r_date) { //significa che l'ultimo ordinamento non era per date
                    ...
                    vettori->caso_heap_insuff = r_date;
            ...
            case r_tratte: {            /**1 Ordinamento per codice tratta*/
                if (vettori->caso_heap_insuff != r_tratte) { //significa che non era ordinato per tratte il vettore
                    ...
                    //siccome lo sto ordinando setto la variabile che si riferisce al fatto se ha ordinato o meno gia il vettore
                    vettori->caso_heap_insuff = r_tratte;
                    /**Per questo tipo di dato preferisco usare l'insertion sort, sempre stabile,in loco*/
                    int i, j, l = 0, r = lunghezza - 1;
                    dati *x;
                    for (i = l + 1; i <= r; i++) {
            ...
            case r_partenza: {            /**2 Ordinamento per stazione di partenza*/
                if (vettori->codice_ordinato != r_partenza) { //significa che non era ordinato per partenza
                    ...
                    //siccome lo sto ordinando setto la variabile che si riferisce al fatto se ha ordinato o meno gia il vettore
                    vettori->caso_heap_insuff = r_partenza;
                    /**Per questo tipo di dato preferisco usare l'insertion sort, sempre stabile,in loco*/
                    int i, j, l = 0, r = lunghezza - 1;
                    dati *x;
                    for (i = l + 1; i <= r; i++) {
            ...
            case r_capolinea: {            /**3 Ordinamento per stazione di arrivo*/
                if (vettori->arrivo_ordinato != r_capolinea) { //significa che non era ordinato per capolinea
                    ...
                    //siccome lo sto ordinando setto la variabile che si riferisce al fatto se ha ordinato o meno gia il vettore
                    vettori->caso_heap_insuff = r_capolinea;
                    /**Per questo tipo di dato preferisco usare l'insertion sort, sempre stabile,in loco*/
                    int i, j, l = 0, r = lunghezza - 1;
                    dati *x;
                    for (i = l + 1; i <= r; i++) {
                        ...
                        j = i - 1;
                        while (j >= l &&
                               strcmp(x->destinazione, (vettori->v_ordinamenti[0][j])->destinazione) < 0) {
                            vettori->v_ordinamenti[0][j + 1] = vettori->v_ordinamenti[0][j];
                            j--;
                        }
                        vettori->v_ordinamenti[0][j + 1] = x;
...
void stampa_log(stampa tipo,comando comando,set_vettori vettori,int lunghezza) {
    ...
    if (Heap_insufficiente == FALSE) {
        ...
        switch (comando) {
            case r_date:
                fprintf(fp,"ORDINAMENTO PER DATE");
                break;
            case r_tratte:
                fprintf(fp,"ORDINAMENTO PER CODICE TRATTA");
                break;
            case r_partenza:
                fprintf(fp,"ORDINAMENTO PER STAZIONE DI PARTENZA");
                break;
            case r_capolinea:
                fprintf(fp,"ORDINAMENTO PER STAZIONE DI ARRIVO");
                break;
            default:
                break;
        }

        for (int i = 0; i < lunghezza; i++) { //stampo direttamente la prima riga della matrice
            fprintf(fp,
                    "\nCodice Tratta: %s || Partenza: %s || Capolinea: %s || Data: %s || Ora partenza: %s ||"
                    "Ora arrivo: %s ", (*(vettori.v_ordinamenti[0][i])).codice_tratta,
                    (*(vettori.v_ordinamenti[0][i])).partenza,
                    (*(vettori.v_ordinamenti[0][i])).destinazione,
                    (*(vettori.v_ordinamenti[0][i])).data_part,
                    (*(vettori.v_ordinamenti[0][i])).orario_partenza,
                    (*(vettori.v_ordinamenti[0][i])).orario_arrivo);
        }
    }
    if (fp != stdout)
        fclose(fp);
...
void ricerca_dicotomica_noHeap(set_vettori vettori,int lunghezza,char chiave[__MAX_S__],int l,int r) {
    int m = (r-l)/2;
    int risultato = strncmp(chiave,vettori.v_ordinamenti[0][m]->partenza,strlen(chiave));
    ...
    if ( risultato > 0)
        ...
        printf("\nRISULTATI RICERCA: ");
        fprintf(stdout, "\nCodice Tratta: %s || Partenza: %s || Capolinea: %s || Data: %s || Ora partenza: %s ||"
                        "Ora arrivo: %s ", (*(vettori.v_ordinamenti[0][m])).codice_tratta,
                (*(vettori.v_ordinamenti[0][m])).partenza,
                (*(vettori.v_ordinamenti[0][m])).destinazione,
                (*(vettori.v_ordinamenti[0][m])).data_part, (*(vettori.v_ordinamenti[0][m])).orario_partenza,
                (*(vettori.v_ordinamenti[0][m])).orario_arrivo);
        int tmp = m;
        while (strncmp(chiave, vettori.v_ordinamenti[0][++m]->partenza, strlen(chiave)) == 0)
        {    fprintf(stdout, "\nCodice Tratta: %s || Partenza: %s || Capolinea: %s || Data: %s || Ora partenza: %s ||"
                             "Ora arrivo: %s ", (*(vettori.v_ordinamenti[0][m])).codice_tratta,
                     (*(vettori.v_ordinamenti[0][m])).partenza,
                     (*(vettori.v_ordinamenti[0][m])).destinazione,
